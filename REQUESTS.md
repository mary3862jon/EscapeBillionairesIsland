...
- [x] **CHEAT CODES (GTA-style typed codes)** — compile-reviewed (no compiler on host; brace-balanced + every API checked against existing code). New `CheatConsole` (type the word anywhere in gameplay; toast + sound + small input line so it's readable). Codes: `tokens` (max Troll Tokens), `goldspoon` (unlock/equip all spoons → Gold), `heat` (toggle Police Mode), `humble` (win nearest boss), `nextlevel` (JumpToNextLevel), `bossquiet` (silence all bosses), `bossrage` (force-attack mode), `bossally` (bosses fight FOR you), `bossnormal` (reset boss mode). Plus `SceneFlow.JumpToNextLevel()` cycling CutleryChamber→SampleScene→SharkFusion.
